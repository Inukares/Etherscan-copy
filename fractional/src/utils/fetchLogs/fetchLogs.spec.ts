...
describe(fetchLogs, () => {
  it('fethes logs and blocks', async () => {
    ...
    const expectedBlocks: { [key: string]: ethers.providers.Block } = {
      '15868070': {
        ...
        difficulty: 0,
        // @ts-ignore -> ethers recognizes gas related properties as BigNumber. Should be safe to ignore it, and better have this typing than 'any'
  ...
  it('ignores failed promises', async () => {
    ...
    const { logs, blocks } = await fetchLogs({
      ...
      provider,
      contractAddress: '0x',
      collectedBlocksMap: expectedBlocks,
